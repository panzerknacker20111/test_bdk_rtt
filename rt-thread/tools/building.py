...
class Win32Spawn:
    def spawn(self, sh, escape, cmd, args, env):
        ...
        try:
            ...
            print('Exception: {}: {}'.format(e, os.strerror(e.errno)))
            return e.errno
        finally:
...
def DoSystemCall(command_string):
    ...
    if (result>>8) != 0:
        print('system({}) error!'.format(command_string))
        exit (1)
    else:
        print('system({}) succeed'.format(command_string))
...
def PrepareBuilding(env, root_directory, has_libcpu=False, remove_components = []):
    ...
    if tgt_name:
        ...
        try:
            ...
            print('Unknow target: {}. Avaible targets: {}'.format(
...
def BuildLibInstallAction(target, source, env):
    ...
    for Group in Projects:
        if Group['name'] == lib_name:
            ...
            print('Copy {} => {}'.format(lib_name, dst_name))
...
def DoBuilding(env, target, objects):
    ...
    if GetOption('buildlib'):
        ...
        for Group in Projects:
            if Group['name'] == lib_name:
                print('buildlib {} '.format(lib_name))
                ...
                if Group['src'] != []:
                    if 'LIBNAME' in Group:
                        ...
                        if os.path.exists(dst_name):
                            print('Remove library: {}'.format(dst_name))
                            os.unlink(dst_name)
...
def EndBuilding(target, program = None):
    ...
    if not GetOption('help') and not GetOption('target'):
        if not os.path.exists(rtconfig.EXEC_PATH):
            print("Error: the toolchain path ({}) is not exist, please check 'EXEC_PATH' in path or rtconfig.py.".format(rtconfig.EXEC_PATH))
            need_exit = True
...
def GetVersion():
    ...
    version = int(''.join(filter(lambda ch: ch in '0123456789.', def_ns['RT_VERSION'])))
    subversion = int(''.join(filter(lambda ch: ch in '0123456789.', def_ns['RT_SUBVERSION'])))

    if 'RT_REVISION' in def_ns:
        revision = int(''.join(filter(lambda ch: ch in '0123456789.', def_ns['RT_REVISION'])))
