...
struct rt_mtd_nand_driver_ops
{
    ...
    rt_err_t (*erase_block)(struct rt_mtd_nand_device* device, uint32_t block);
    rt_err_t (*check_block)(struct rt_mtd_nand_device* device, uint32_t block);
    rt_err_t (*mark_badblock)(struct rt_mtd_nand_device* device, uint32_t block);
};

rt_err_t rt_mtd_nand_register_device(const char* name, struct rt_mtd_nand_device* device);

rt_inline uint32_t rt_mtd_nand_read_id(struct rt_mtd_nand_device* device)
{
...
rt_inline rt_err_t rt_mtd_nand_erase_block(struct rt_mtd_nand_device* device, uint32_t block)
{
    return device->ops->erase_block(device, block);
}

rt_inline rt_err_t rt_mtd_nand_check_block(struct rt_mtd_nand_device* device, uint32_t block)
{
    return device->ops->check_block(device, block);
}

rt_inline rt_err_t rt_mtd_nand_mark_badblock(struct rt_mtd_nand_device* device, uint32_t block)
{
