...
struct rt_hwtimer_ops
{
    ...
    rt_err_t (*start)(struct rt_hwtimer_device *timer, uint32_t cnt, rt_hwtimer_mode_t mode);
    void (*stop)(struct rt_hwtimer_device *timer);
    uint32_t (*count_get)(struct rt_hwtimer_device *timer);
    rt_err_t (*control)(struct rt_hwtimer_device *timer, uint32_t cmd, void *args);
};
