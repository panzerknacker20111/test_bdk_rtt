...
static const struct kernel_task_desc TASK_BLE_DESC_DBG = {NULL, &dbg_default_handler, dbg_state, DBG_STATE_MAX, DBG_IDX_MAX};
#endif //RW_DEBUG
...
{
    // Create DEBUG Task
    kernel_task_create(TASK_BLE_DBG, &TASK_BLE_DESC_DBG);

    // Initialize DBG task to idle state
    kernel_state_set(TASK_BLE_DBG,DBG_IDLE);
