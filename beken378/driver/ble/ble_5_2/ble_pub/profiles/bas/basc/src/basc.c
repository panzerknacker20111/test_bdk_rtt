 ...
 ****************************************************************************************
 */
__STATIC void basc_read_val_cmp(uint8_t conidx, uint16_t status, uint16_t dummy, common_buf_t* p_data)
{
    ...
    if(p_basc_env != NULL)
    {
        ...
        switch(val_id)
        {
            case BASC_BATT_LVL_VAL:
            {
                ...
                {
                    batt_level = common_buf_data(p_data)[0];
                }
    ...
    if(p_basc_env != NULL)
    {
        ...
        if(p_con_env != NULL)
        {
            ...
            for (bas_inst = 0; (bas_inst < p_con_env->nb_svc); bas_inst++)
            {
                ...
                {
                    uint8_t batt_level = common_buf_data(p_data)[0];
...
uint16_t basc_enable(uint8_t conidx, uint8_t con_type, uint8_t nb_bas, const bas_content_t* p_bas)
{
    ...
    else if(p_basc_env != NULL)
    {
        if ((conidx < BLE_CONNECTION_MAX) && (p_basc_env->p_env[conidx] == NULL))
        {
            ...
            p_basc_env->p_env[conidx] = (basc_cnx_env_t *) kernel_malloc(sizeof(basc_cnx_env_t), KERNEL_MEM_ATT_DB);
 ...
 ****************************************************************************************
 */
__STATIC int basc_enable_req_handler(kernel_msg_id_t const msgid, struct basc_enable_req const *p_param,
                                     kernel_task_id_t const dest_id, kernel_task_id_t const src_id)
{
    ...
    if (status != GAP_ERR_NO_ERROR)
    {
        struct basc_enable_rsp *p_rsp = KERNEL_MSG_ALLOC(BASC_ENABLE_RSP, src_id, dest_id, basc_enable_rsp);
        if(p_rsp != NULL)
        {
 ...
 ****************************************************************************************
 */
__STATIC int basc_read_info_req_handler(kernel_msg_id_t const msgid, struct basc_read_info_req const *p_param,
                                        kernel_task_id_t const dest_id, kernel_task_id_t const src_id)
{
    uint16_t status;
    ...
    if (status != GAP_ERR_NO_ERROR)
    {
        struct basc_read_info_rsp *p_rsp = KERNEL_MSG_ALLOC(BASC_READ_INFO_RSP, src_id, dest_id, basc_read_info_rsp);
 ...
 ****************************************************************************************
 */
__STATIC int basc_batt_level_ntf_cfg_req_handler(kernel_msg_id_t const msgid, struct basc_batt_level_ntf_cfg_req const *p_param,
                                                 kernel_task_id_t const dest_id, kernel_task_id_t const src_id)
{
    ...
    if (status != GAP_ERR_NO_ERROR)
    {
        struct basc_batt_level_ntf_cfg_rsp *p_rsp = KERNEL_MSG_ALLOC(BASC_BATT_LEVEL_NTF_CFG_RSP, src_id, dest_id,
...
void basc_cb_enable_cmp(uint8_t conidx, uint16_t status, uint8_t bas_nb, const bas_content_t* p_bas)
{
    ...
    struct basc_enable_rsp *p_rsp = KERNEL_MSG_ALLOC(BASC_ENABLE_RSP, PRF_DST_TASK(BASC), PRF_SRC_TASK(BASC),
                                                 basc_enable_rsp);
    if(p_rsp != NULL)
    {
        p_rsp->conidx = conidx;
...
{
    struct basc_read_info_rsp *p_rsp = KERNEL_MSG_ALLOC(BASC_READ_INFO_RSP, PRF_DST_TASK(BASC),
...
{
    struct basc_batt_level_ntf_cfg_rsp *p_rsp = KERNEL_MSG_ALLOC(BASC_BATT_LEVEL_NTF_CFG_RSP, PRF_DST_TASK(BASC),
...
void basc_cb_batt_level_upd(uint8_t conidx, uint8_t bas_instance, uint8_t batt_level)
{
    struct basc_batt_level_ind *p_ind =
            KERNEL_MSG_ALLOC(BASC_BATT_LEVEL_IND, PRF_DST_TASK(BASC), PRF_SRC_TASK(BASC), basc_batt_level_ind);
                          ...
                          const void* p_params, const basc_cb_t* p_cb)
{
    ...
    do
    {
        ...
        p_basc_env = (basc_env_t*) kernel_malloc(sizeof(basc_env_t), KERNEL_MEM_ATT_DB);
...
__STATIC uint16_t basc_destroy(prf_data_t *p_env, uint8_t reason)
{
    ...
    if(status == GAP_ERR_NO_ERROR)
    {
        ...
        for (idx = 0; idx < BLE_CONNECTION_MAX; idx++)
        {
            ...
            {
                kernel_free(p_basc_env->p_env[idx]);
            }
        }

        // free profile environment variables
        p_env->p_env = NULL;
        kernel_free(p_basc_env);
    }
 ...
 ****************************************************************************************
 */
__STATIC void basc_con_create(prf_data_t *p_env, uint8_t conidx, const gap_con_param_t* p_con_param)
{
    // Nothing to do
}
...
__STATIC void basc_con_cleanup(prf_data_t *p_env, uint8_t conidx, uint16_t reason)
{
    ...
    if (p_basc_env->p_env[conidx] != NULL)
    {
        kernel_free(p_basc_env->p_env[conidx]);
        p_basc_env->p_env[conidx] = NULL;
    }
}
