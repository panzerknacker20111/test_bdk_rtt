...
rt_inline rt_uint32_t alarm_mkdaysec(struct tm_at *time)
{
    rt_uint32_t sec;
...
static void alarm_update(rt_uint32_t event)
{
    ...
    {
        /* get time of now */
        timestamp = time_at(RT_NULL);
        localtime_r_at(&timestamp, &now);
...
rt_err_t rt_alarm_start(rt_alarm_t alarm)
{
    ...
    {
        timestamp = time1(RT_NULL);
        localtime_r1(&timestamp, &now);
