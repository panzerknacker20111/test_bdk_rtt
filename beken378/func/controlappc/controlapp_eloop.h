...
/* Magic number for controlapp_eloop_cancel_timeout() */
#define ELOOP_ALL_CTX (void *) -1

/**
 * controlapp_eloop_init() - Initialize global event loop data
 * @user_data: Pointer to global data passed as eloop_ctx to signal handlers
 *
 * This function must be called before any other controlapp_eloop_* function. user_data
...
/**
 * controlapp_eloop_register_read_sock - Register handler for read events
...
/**
 * controlapp_eloop_unregister_read_sock - Unregister handler for read events
 * @sock: File descriptor number for the socket
 *
 * Unregister a read socket notifier that was previously registered with
 * controlapp_eloop_register_read_sock().
 */
void controlapp_eloop_unregister_read_sock(int sock);

/**
 * controlapp_eloop_register_timeout - Register timeout
...
/**
 * controlapp_eloop_cancel_timeout - Cancel timeouts
 ...
 * controlapp_eloop_register_timeout(). ELOOP_ALL_CTX can be used as a wildcard for
...
/**
 * controlapp_eloop_register_signal - Register handler for signals
 ...
 * controlapp_eloop_init() will be used as eloop_ctx for signal handlers.
 */
...
/**
 * controlapp_eloop_run - Start the event loop
 *
...
/**
 * controlapp_eloop_terminate - Terminate event loop
 *
...
/**
 * controlapp_eloop_destroy - Free any resources allocated for the event loop
 *
 * After calling controlapp_eloop_destoy(), other controlapp_eloop_* functions must not be called
 * before re-running controlapp_eloop_init().
 */
void controlapp_eloop_destroy(void);

/**
 * controlapp_eloop_terminated - Check whether event loop has been terminated
 * Returns: 1 = event loop terminate, 0 = event loop still running
 *
 * This function can be used to check whether controlapp_eloop_terminate() has been called
 * to request termination of the event loop. This is normally used to abort
 * operations that may still be queued to be run when controlapp_eloop_terminate() was
 * called.
 */
