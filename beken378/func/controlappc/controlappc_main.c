...
#define CONTROL_APPC_UART            BK_UART_2
...
static void control_appc_uart_rx_callback(int uport, void *param)
{
...
void uart_send_bytes(uint8_t port, const void *buf, int len)
{
	// print_hex_dump("UART: ", buf, len);

	const uint8_t *b = buf;
	for (int i = 0; i < len; i++) {
		//bk_printf("%02X ", b[i]);
		bk_send_byte(port, b[i]);
	}
...
static void control_receive_message(int sock, void *eloop_ctx, void *sock_ctx) {
    ...
    if (ret == 0) {
        ...
        uart_send_bytes(CONTROL_APPC_UART, buffer, len);
        goto done;
    }
...
{
	while (bk_uart_recv(CONTROL_APPC_UART, &inbuf[*bp], 1, BEKEN_WAIT_FOREVER) == 0) {
		(*bp)++;
		if (*bp >= bufLen) {
...
int controlappc_main(int argc, char* argv[]) {
    ...
    */
	...
	bk_uart_set_rx_callback(CONTROL_APPC_UART, control_appc_uart_rx_callback, NULL);
	...
	for (;;) {
		while (1) {
			...
			/* get input from uart */
			wfaCtrlGetInput((char *)control_appc_uart_rx_buf, CONTROL_APPC_UART_BUF_SZ, &uart_rx_pos);
