...
static int _mbedtls_client_init(DTLSDataParams *pDataParams, DTLSConnectParams *pConnectParams)
{
    ...
    if ((ret = mbedtls_ctr_drbg_seed(&pDataParams->ctr_drbg, mbedtls_entropy_func, &pDataParams->entropy, NULL, 0)) !=
        0) {
        ATSVRLOG("mbedtls_ctr_drbg_seed failed returned -0x%x", -ret);
    ...
    if (pConnectParams->ca_crt != NULL) {
        if ((ret = mbedtls_x509_crt_parse(&(pDataParams->ca_cert), (const unsigned char *)pConnectParams->ca_crt,
            ...
            ATSVRLOG("parse ca crt failed returned -0x%04x", -ret);
    ...
    if (pConnectParams->cert_file != NULL && pConnectParams->key_file != NULL) {
        if ((ret = mbedtls_x509_crt_parse_file(&(pDataParams->client_cert), pConnectParams->cert_file)) != 0) {
            ATSVRLOG("load client cert file failed returned -0x%x", ret);
            return ATSVR_ERR_SSL_CERT;
        }

        if ((ret = mbedtls_pk_parse_keyfile(&(pDataParams->private_key), pConnectParams->key_file, "")) != 0) {
            ATSVRLOG("load client key file failed returned -0x%x", ret);
            return ATSVR_ERR_SSL_CERT;
        }
        ...
        if (0 == ret) {
            ...
            if ((ret = mbedtls_ssl_conf_own_cert(&(pDataParams->ssl_conf), &(pDataParams->client_cert),
                ...
                ATSVRLOG("mbedtls_ssl_conf_own_cert failed returned -0x%x", -ret);
                return ATSVR_ERR_SSL_CERT;
            }
        }
    } else {
        ATSVRLOG("cert_file/key_file is empty!|cert_file=%s|key_file=%s",
    ...
    if (pConnectParams->psk != NULL && pConnectParams->psk_id != NULL) {
        ...
        ATSVRLOG("psk/pskid is empty!");
    }

    if (0 != ret) {
        ATSVRLOG("mbedtls_ssl_conf_psk fail: -0x%x", -ret);
        return ret;
    }
#endif
...
int _mbedtls_udp_connect(mbedtls_net_context *socket_fd, const char *host, int port)
{
    ...
    if ((ret = mbedtls_net_connect(socket_fd, host, port_str, MBEDTLS_NET_PROTO_UDP)) != 0) {
        ATSVRLOG("mbedtls_net_connect host:%s port:%s returned -0x%04x errno: %d", STRING_PTR_PRINT_SANITY_CHECK(host),
              port_str, -ret, errno);
        switch (ret) {
    ...
    if ((ret = mbedtls_net_set_block(socket_fd)) != 0) {
        ATSVRLOG("set block faliled returned -0x%04x", -ret);
        return ATSVR_ERR_TCP_CONNECT;
    }
...
uintptr_t HAL_DTLS_Connect(DTLSConnectParams *pConnectParams, const char *host, int port)
{
    ...
    DTLSDataParams *pDataParams = (DTLSDataParams *)at_malloc(sizeof(DTLSDataParams));

    if ((ret = _mbedtls_client_init(pDataParams, pConnectParams)) != ATSVR_RET_SUCCESS) {
        goto error;
    }

    if ((ret = _mbedtls_udp_connect(&(pDataParams->socket_fd), host, port)) != ATSVR_RET_SUCCESS) {
        goto error;
    }
    ...
    if ((ret = mbedtls_ssl_config_defaults(&pDataParams->ssl_conf, MBEDTLS_SSL_IS_CLIENT,
        ...
        ATSVRLOG("mbedtls_ssl_config_defaults result 0x%04x", ret);
        goto error;
    }
    ...
    if ((ret = mbedtls_ssl_cookie_setup(&pDataParams->cookie_ctx, mbedtls_ctr_drbg_random, &pDataParams->ctr_drbg)) !=
        0) {
        ATSVRLOG("mbedtls_ssl_cookie_setup result 0x%04x", ret);
        goto error;
    }
    ...
    if ((ret = mbedtls_ssl_setup(&(pDataParams->ssl), &(pDataParams->ssl_conf))) != 0) {
        ATSVRLOG("mbedtls_ssl_setup failed returned -0x%x", -ret);
        goto error;
    }
    ...
    if ((ret = mbedtls_ssl_set_hostname(&(pDataParams->ssl), host)) != 0) {
        ATSVRLOG("mbedtls_ssl_set_hostname failed returned -0x%x", -ret);
        goto error;
    }
    ...
    while ((ret = mbedtls_ssl_handshake(&(pDataParams->ssl))) != 0) {
        if (ret != MBEDTLS_ERR_SSL_WANT_READ && ret != MBEDTLS_ERR_SSL_WANT_WRITE) {
            ATSVRLOG("mbedtls_ssl_handshake failed returned -0x%x", -ret);
            if (ret == MBEDTLS_ERR_X509_CERT_VERIFY_FAILED) {
                ATSVRLOG("Unable to verify the server's certificate");
            }
            goto error;
        }
    }
    ...
    if ((ret = mbedtls_ssl_get_verify_result(&(pDataParams->ssl))) != 0) {
        ATSVRLOG("mbedtls_ssl_get_verify_result failed returned -0x%x", -ret);
        goto error;
    }
...
int HAL_DTLS_Write(uintptr_t handle, const unsigned char *data, size_t datalen, size_t *written_len)
{
    ...
    POINTER_SANITY_CHECK(data_params, ATSVR_ERR_INVAL);
    POINTER_SANITY_CHECK(data, ATSVR_ERR_INVAL);
    POINTER_SANITY_CHECK(written_len, ATSVR_ERR_INVAL);

    int rc = mbedtls_ssl_write(&data_params->ssl, data, datalen);

    if (rc < 0) {
        ATSVRLOG("failed! mbedtls_ssl_write returned %d", rc);
        rc = ATSVR_ERR_SSL_WRITE;
    }

    *written_len = rc;
    rc           = ATSVR_RET_SUCCESS;
    return rc;
...
int HAL_DTLS_Read(uintptr_t handle, unsigned char *data, size_t datalen, unsigned int timeout_ms, size_t *read_len)
{
    ...
    POINTER_SANITY_CHECK(data_params, ATSVR_ERR_INVAL);
    POINTER_SANITY_CHECK(data, ATSVR_ERR_INVAL);
    POINTER_SANITY_CHECK(read_len, ATSVR_ERR_INVAL);

    int rc = ATSVR_ERR_SSL_READ;
        ...
        *read_len = 0;
        switch (rc) {
            case MBEDTLS_ERR_SSL_TIMEOUT:
                rc = ATSVR_ERR_SSL_READ_TIMEOUT;
                break;

            case MBEDTLS_ERR_SSL_PEER_CLOSE_NOTIFY:
                ATSVRLOG("connection was closed gracefully");
                rc = ATSVR_ERR_DTLS_PEER_CLOSE_NOTIFY;
                break;

            default:
                ATSVRLOG(" mbedtls_ssl_read returned -0x%x", -rc);
                break;
        }
    } else {
        *read_len = rc;
