...
#define NETWORK_ADDR_LEN (16)
    ...
    ATSVRLOG("establish tcp connection with server(host=%s port=%s)", STRING_PTR_PRINT_SANITY_CHECK(host), port_str);

    if (getaddrinfo(host, port_str, &hints, &addr_list) != 0) {
        ATSVRLOG("getaddrinfo error,errno:%s", STRING_PTR_PRINT_SANITY_CHECK(strerror(errno)));
        return 0;
    }
    ...
    if (0 == ret) {
        ATSVRLOG("fail to establish udp");
    } else {
        ATSVRLOG("success to establish udp, fd=%d", ret);
    }
...
int HAL_UDP_Read(uintptr_t fd, unsigned char *p_data, unsigned int datalen)
{
   ...
   // fd -= LWIP_SOCKET_FD_SHIFT;
	...
	ATSVRLOG("socket_id:%d,udp read count:%d\r\n",socket_id,count);
...
int HAL_UDP_ReadTimeout(uintptr_t fd, unsigned char *p_data, unsigned int datalen, unsigned int timeout_ms)
{
    ...
    if (ret == 0) {
        return ATSVR_ERR_SSL_READ_TIMEOUT; /* receive timeout */
    }
    ...
    if (ret < 0) {
        ...
        return ATSVR_ERR_SSL_READ; /* receive failed */
    }
...
#define NETWORK_ADDR_LEN (16)
    ...
    ATSVRLOG("establish udp connection with server(host=%s port=%s)", host, port_str);

    if (getaddrinfo(host, port_str, &hints, &addr_list) != 0) {
        ATSVRLOG("getaddrinfo error,errno:%s", strerror(errno));
        return 0;
    }
    ...
    for (cur = addr_list; cur != NULL; cur = cur->ai_next) {
        ...
        ATSVRLOG("establish udp connection with server(host=%d port=%d)", ((struct sockaddr_in *)cur->ai_addr)->sin_addr,
    ...
    ATSVRLOG("HAL_UDP_Connect fd:%d", ret);
                                ...
                                char *recv_ip_addr, unsigned char recv_addr_len, unsigned short *recv_port)
{
 ...
 //   unsigned int       addrLen = sizeof(source_addr);
    socklen_t          srvaddr_len = (socklen_t)sizeof(struct sockaddr_in);;
    int                len = 0;

    ATSVRLOG("HAL_UDP_ReadTimeoutPeerInfo fd:%d", fd);
    ...
    len = recvfrom((int)fd, p_data, datalen, MSG_DONTWAIT, (struct sockaddr *)&source_addr, &srvaddr_len);
...
int HAL_UDP_WriteTo(uintptr_t fd, const unsigned char *p_data, unsigned int datalen, char *host, unsigned short port)
{
    ...
    hints.ai_family   = AF_INET;
    hints.ai_protocol = IPPROTO_UDP;
    ATSVRLOG("HAL_UDP_WriteTo host:%s, port:%d, fd:%d\r\n", host, port, fd);

    if ((rc = getaddrinfo(host, port_str, &hints, &addr_list)) != 0) {
        ATSVRLOG("getaddrinfo error,errno:%s, rc:%d", strerror(errno), rc);
        return 0;
    }

 //   ATSVRLOG("HAL_UDP_WriteTo(host=%d port=%d)", ((struct sockaddr_in *)addr_list->ai_addr)->sin_addr,
