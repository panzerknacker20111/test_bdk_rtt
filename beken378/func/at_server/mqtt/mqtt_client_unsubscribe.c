...
int qcloud_iot_mqtt_unsubscribe(Qcloud_IoT_Client *pClient, char *topicFilter)
{
    ...
    for (i = 0; i < MAX_MESSAGE_HANDLERS; ++i) {
        if ((pClient->sub_handles[i].topic_filter != NULL &&
            ...
            at_free((void *)pClient->sub_handles[i].topic_filter);
    ...
    if (suber_exists == false) {
        ATSVRLOG("subscription does not exists: %s", topicFilter);
        AT_FUNC_EXIT_RC(ATSVR_ERR_MQTT_UNSUB_FAIL);
    }
    ...
    if (ATSVR_RET_SUCCESS != rc) {
        ATSVRLOG("push publish into to pubInfolist failed: %d", rc);
        HAL_MutexUnlock(pClient->lock_write_buf);
        at_free(topic_filter_stored);
        AT_FUNC_EXIT_RC(rc);
    }
