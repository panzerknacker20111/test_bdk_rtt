...
uint16_t get_next_packet_id(Qcloud_IoT_Client *pClient)
{
    ...
    AT_FUNC_EXIT_RC(pClient->next_packet_id);
}
    ...
    *value                   = 0;
    do {
        if (++len > MAX_NO_OF_REMAINING_LENGTH_BYTES) {
            /* bad data */
            AT_FUNC_EXIT_RC(ATSVR_ERR_MQTT_PACKET_READ);
        }
        c = *bufptr++;
...
{
    POINTER_SANITY_CHECK(header, ATSVR_ERR_INVAL);
    unsigned char type, qos;

    switch (message_type) {
        case RESERVED:
            /* Should never happen */
            return ATSVR_ERR_MQTT_UNKNOINN;
...
int deserialize_ack_packet(uint8_t *packet_type, uint8_t *dup, uint16_t *packet_id, unsigned char *buf, size_t buf_len)
{
    ...
    POINTER_SANITY_CHECK(packet_type, ATSVR_ERR_INVAL);
    POINTER_SANITY_CHECK(dup, ATSVR_ERR_INVAL);
    POINTER_SANITY_CHECK(packet_id, ATSVR_ERR_INVAL);
    ...
    *packet_id = mqtt_read_uint16_t(&curdata);
    ...
    if (enddata - curdata >= 1) {
        ...
        if (ack_code != 0) {
            ATSVRLOG("deserialize_ack_packet failure! ack_code = 0x%02x", ack_code);
            AT_FUNC_EXIT_RC(ATSVR_ERR_FAILURE);
        }
    }
                              ...
                              unsigned char *buf, size_t buf_len)
{
    ...
    POINTER_SANITY_CHECK(packet_id, ATSVR_ERR_INVAL);
    POINTER_SANITY_CHECK(count, ATSVR_ERR_INVAL);
    POINTER_SANITY_CHECK(grantedQoSs, ATSVR_ERR_INVAL);
    ...
    if (decodeRc != ATSVR_RET_SUCCESS) {
        AT_FUNC_EXIT_RC(decodeRc);
    }
...
int deserialize_unsuback_packet(uint16_t *packet_id, unsigned char *buf, size_t buf_len)
{
    ...
    if (ATSVR_RET_SUCCESS == rc && UNSUBACK != type) {
        rc = ATSVR_ERR_FAILURE;
    }
...
int send_mqtt_packet(Qcloud_IoT_Client *pClient, size_t length, Timer *timer)
{
    ...
    POINTER_SANITY_CHECK(pClient, ATSVR_ERR_INVAL);
    POINTER_SANITY_CHECK(timer, ATSVR_ERR_INVAL);

    int    rc      = ATSVR_ERR_FAILURE;
    ...
    while (sent < length && !HAL_Timer_expired(timer)) {
        rc = pClient->network_stack.netwrite(&(pClient->network_stack), &pClient->write_buf[sent], length, HAL_Timer_remain(timer),
                                          &sentLen);
...
static int _decode_packet_rem_len_with_net_read(Qcloud_IoT_Client *pClient, uint32_t *value, uint32_t timeout)
{
    ...
    POINTER_SANITY_CHECK(pClient, ATSVR_ERR_INVAL);
    POINTER_SANITY_CHECK(value, ATSVR_ERR_INVAL);
    ...
    *value = 0;
    ...
    do {
        ...
        if ((pClient->network_stack.netread(&(pClient->network_stack), &i, 1, timeout, &read_len)) != ATSVR_RET_SUCCESS) {
...
static int _read_mqtt_packet(Qcloud_IoT_Client *pClient, Timer *timer, uint8_t *packet_type)
{
    ...
    size_t   read_len = 0;
    int      rc;
    int      timer_left_ms = HAL_Timer_remain(timer);
    ...
    // 1. read 1st byte in fixed header and check if valid
    rc = pClient->network_stack.netread(&(pClient->network_stack), pClient->read_buf, 1, timer_left_ms, &read_len);
    if (rc == ATSVR_ERR_SSL_NOTHING_TO_READ || rc == ATSVR_ERR_TCP_NOTHING_TO_READ) {
        AT_FUNC_EXIT_RC(ATSVR_ERR_MQTT_NOTHING_TO_READ);
    ...
    // 2. read the remaining length
    timer_left_ms = HAL_Timer_remain(timer);
    if (timer_left_ms <= 0) {
        timer_left_ms = 1;
    }
    ...
    if (rem_len >= pClient->read_buf_size) {
        ...
        do {
            ret_val = pClient->network_stack.netread(&(pClient->network_stack), pClient->read_buf, bytes_to_be_read,
                                                  timer_left_ms, &read_len);
            if (ret_val == ATSVR_RET_SUCCESS) {
        ...
        } while (total_bytes_read < rem_len && ret_val == ATSVR_RET_SUCCESS);

        ATSVRLOG("MQTT Recv buffer not enough: %d < %d", pClient->read_buf_size, rem_len);
        AT_FUNC_EXIT_RC(ATSVR_ERR_BUF_TOO_SHORT);
    }
    ...
    if (rem_len > 0 && ((len + rem_len) > pClient->read_buf_size)) {
        ...
        pClient->network_stack.netread(&(pClient->network_stack), pClient->read_buf, rem_len, timer_left_ms, &read_len);
        AT_FUNC_EXIT_RC(ATSVR_ERR_BUF_TOO_SHORT);
    } else {
        if (rem_len > 0) {
            timer_left_ms = HAL_Timer_remain(timer);
            ...
            timer_left_ms += QCLOUD_IOT_MQTT_MAX_REMAIN_WAIT_MS;
            rc = pClient->network_stack.netread(&(pClient->network_stack), pClient->read_buf + len, rem_len, timer_left_ms,
                                             &read_len);
            if (rc != ATSVR_RET_SUCCESS) {
                AT_FUNC_EXIT_RC(rc);
            }
...
static int _deliver_message(Qcloud_IoT_Client *pClient, char *topicName, uint16_t topicNameLen, MQTTMessage *message)
{
    ...
    POINTER_SANITY_CHECK(pClient, ATSVR_ERR_INVAL);
    POINTER_SANITY_CHECK(topicName, ATSVR_ERR_INVAL);
    POINTER_SANITY_CHECK(message, ATSVR_ERR_INVAL);
    ...
    ATSVRLOG("no matching any topic, call default handle function");
...
static int _handle_suback_packet(Qcloud_IoT_Client *pClient, Timer *timer, QoS qos)
{
    ...
    if (/*(NULL == sub_handle.message_handler) || */ (NULL == sub_handle.topic_filter)) {
        ATSVRLOG("sub_handle is illegal, topic is null");
        HAL_MutexUnlock(pClient->lock_generic);
        AT_FUNC_EXIT_RC(ATSVR_ERR_MQTT_SUB);
    }

    if (sub_nack) {
        HAL_MutexUnlock(pClient->lock_generic);
        ATSVRLOG("MQTT SUBSCRIBE failed, packet_id: %u topic: %s", packet_id, sub_handle.topic_filter);
        ...
        at_free((void *)sub_handle.topic_filter);
    ...
    for (i = 0; i < MAX_MESSAGE_HANDLERS; ++i) {
        if ((NULL != pClient->sub_handles[i].topic_filter)) {
            if (0 == _check_handle_is_identical(&pClient->sub_handles[i], &sub_handle)) {
                flag_dup = 1;
                ATSVRLOG("Identical topic found: %s", sub_handle.topic_filter);
                if (pClient->sub_handles[i].handler_user_data != sub_handle.handler_user_data) {
                    ATSVRLOG("Update handler_user_data %p -> %p!", pClient->sub_handles[i].handler_user_data,
    ...
    if (0 == flag_dup) {
        if (-1 == i_free) {
            ATSVRLOG("NO more @sub_handles space!");
            HAL_MutexUnlock(pClient->lock_generic);
            AT_FUNC_EXIT_RC(ATSVR_ERR_FAILURE);
        } else {
...
static int _handle_unsuback_packet(Qcloud_IoT_Client *pClient, Timer *timer)
{
    ...
    if (messageHandler.topic_filter) {
        at_free((void *)messageHandler.topic_filter);
...
static int _handle_publish_packet(Qcloud_IoT_Client *pClient, Timer *timer)
{
    ...
    if (topic_len > MAX_SIZE_OF_CLOUD_TOPIC) {
        ...
        ATSVRLOG("topic len exceed buffer len");
    }
    ...
    if (HAL_Timer_expired(timer)) {
        ...
        HAL_Timer_countdown_ms(timer, 100);
    }
...
int cycle_for_read(Qcloud_IoT_Client *pClient, Timer *timer, uint8_t *packet_type, QoS qos)
{
    ...
    if (ATSVR_ERR_MQTT_NOTHING_TO_READ == rc) {
    ...
    switch (*packet_type) {
        ...
        case PUBREL: {
            ATSVRLOG("Packet type PUBREL is currently NOT handled!");
            break;
        }
        case PUBCOMP:
            break;
        ...
        default: {
            ...
            AT_FUNC_EXIT_RC(ATSVR_ERR_RX_MESSAGE_INVAL);
            break;
...
int wait_for_read(Qcloud_IoT_Client *pClient, uint8_t packet_type, Timer *timer, QoS qos)
{
    ...
    do {
        if (HAL_Timer_expired(timer)) {
            rc = ATSVR_ERR_MQTT_REQUEST_TIMEOUT;
            break;
        }
        rc = cycle_for_read(pClient, timer, &read_packet_type, qos);
    } while (ATSVR_RET_SUCCESS == rc && read_packet_type != packet_type);
                     ...
                     ListNode **node)
{
    ...
    if (c->list_sub_wait_ack->len >= MAX_MESSAGE_HANDLERS) {
        HAL_MutexUnlock(c->lock_list_sub);
        ATSVRLOG("number of sub_info more than max! size = %d", c->list_sub_wait_ack->len);
        AT_FUNC_EXIT_RC(ATSVR_ERR_MQTT_MAX_SUBSCRIPTIONS);
    }

    QcloudIotSubInfo *sub_info = (QcloudIotSubInfo *)at_malloc(sizeof(QcloudIotSubInfo) + len);
    if (NULL == sub_info) {
        HAL_MutexUnlock(c->lock_list_sub);
        ATSVRLOG("malloc failed!");
        AT_FUNC_EXIT_RC(ATSVR_ERR_FAILURE);
    }
    ...
    HAL_Timer_init(&sub_info->sub_start_time);
    HAL_Timer_countdown_ms(&sub_info->sub_start_time, c->command_timeout_ms);
