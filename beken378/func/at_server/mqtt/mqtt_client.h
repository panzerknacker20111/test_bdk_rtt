...
#define ATSVR_IOT_DEVICE_SDK_APPID_LEN  (32)
...
#define MAX_HOSTNAME_SIZE_LEN  (64)

/* default MQTT Tx buffer size, MAX: 16*1024 */
#define ATSVR_IOT_MQTT_TX_BUF_LEN (2048)

/* default MQTT Rx buffer size, MAX: 16*1024 */
#define ATSVR_IOT_MQTT_RX_BUF_LEN (2048)
...
#define ATSVR_IOT_AT_VERSION "AT_BK_v1.0.0"
#define MAX_SIZE_OF_PUBL_PAYLOAD 512
...
#define ATSEVER_TLS_HANDSHAKE_TIMEOUT (5 * 1000)
...
typedef struct Client {
    ...
    unsigned char write_buf[MAX_DATA_BUF_LEN];  // MQTT write buffer
    unsigned char read_buf[MAX_DATA_BUF_LEN];   // MQTT read buffer
    ...
    ATNetwork network_stack;  // MQTT network stack
    ...
    char host_addr[MAX_HOSTNAME_SIZE_LEN];
...
#else
    unsigned char psk_decode[DECODE_PSK_LENGTH+1];
#endif
