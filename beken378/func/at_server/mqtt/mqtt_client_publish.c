...
static int _read_string_with_len(char **string, uint16_t *stringLen, unsigned char **pptr, unsigned char *enddata)
{
    ...
    if (enddata - (*pptr) > 1) {
        *stringLen = mqtt_read_uint16_t(pptr); /* increments pptr to point past length */
        ...
        if (*stringLen > ATSVR_IOT_MQTT_RX_BUF_LEN) {
            ATSVRLOG("stringLen exceed ATSVR_IOT_MQTT_RX_BUF_LEN");
...
static int _mask_push_pubInfo_to(Qcloud_IoT_Client *c, int len, unsigned short msgId, ListNode **node)
{
    ...
    if (!c || !node) {
        ATSVRLOG("invalid parameters!");
        AT_FUNC_EXIT_RC(ATSVR_ERR_MQTT_PUSH_TO_LIST_FAILED);
    }

    if ((len < 0) || (len > c->write_buf_size)) {
        ATSVRLOG("the param of len is error!");
        AT_FUNC_EXIT_RC(ATSVR_ERR_FAILURE);
    }
    ...
    if (c->list_pub_wait_ack->len >= MAX_REPUB_NUM) {
        ...
        ATSVRLOG("more than %u elements in republish list. List overflow!", c->list_pub_wait_ack->len);
        AT_FUNC_EXIT_RC(ATSVR_ERR_FAILURE);
    }

    QcloudIotPubInfo *repubInfo = (QcloudIotPubInfo *)at_malloc(sizeof(QcloudIotPubInfo) + len);
    if (NULL == repubInfo) {
        HAL_MutexUnlock(c->lock_list_pub);
    ...
    repubInfo->len        = len;
    HAL_Timer_init(&repubInfo->pub_start_time);
    HAL_Timer_countdown_ms(&repubInfo->pub_start_time, c->command_timeout_ms);
                               ...
                               size_t buf_len)
{
    ...
    POINTER_SANITY_CHECK(qos, ATSVR_ERR_INVAL);
    POINTER_SANITY_CHECK(retained, ATSVR_ERR_INVAL);
    ...
    *retained = header & MQTT_HEADER_RETAIN_MASK;
    ...
    if (ATSVR_RET_SUCCESS != _read_string_with_len(topicName, topicNameLen, &curdata, enddata) ||
        (0 > (enddata - curdata))) {
        AT_FUNC_EXIT_RC(ATSVR_ERR_FAILURE);
    }
                                     ...
                                     uint32_t *serialized_len)
{
    ...
    POINTER_SANITY_CHECK(buf, ATSVR_ERR_INVAL);
    POINTER_SANITY_CHECK(serialized_len, ATSVR_ERR_INVAL);
    POINTER_SANITY_CHECK(payload, ATSVR_ERR_INVAL);
...
int qcloud_iot_mqtt_publish(Qcloud_IoT_Client *pClient, char *topicName, PublishParams *pParams)
{
    ...
    STRING_PTR_SANITY_CHECK(topicName, ATSVR_ERR_INVAL);
    ...
    if (topicLen > MAX_SIZE_OF_CLOUD_TOPIC) {
        AT_FUNC_EXIT_RC(ATSVR_ERR_MAX_TOPIC_LENGTH);
    }

    if (pParams->qos == QOS2) {
        ATSVRLOG("QoS2 is not supported currently");
        AT_FUNC_EXIT_RC(ATSVR_ERR_MQTT_QOS_NOT_SUPPORT);
    }
    ...
    if (pParams->qos == QOS1) {
      ...
      /*  if (IOT_Log_Get_Level() <= eLOG_DEBUG) {
            ATSVRLOG("publish topic seq=%d|topicName=%s|payload=%s", pParams->id, topicName,
                  STRING_PTR_PRINT_SANITY_CHECK((char *)pParams->payload));
        } else {
            ATSVRLOG("publish topic seq=%d|topicName=%s", pParams->id, topicName);
        }*/
         ATSVRLOG("publish topic seq=%d|topicName=%s|payload=%s", pParams->id, topicName,
                  STRING_PTR_PRINT_SANITY_CHECK((char *)pParams->payload));
    } else {
       /* if (IOT_Log_Get_Level() <= eLOG_DEBUG) {
            ATSVRLOG("publish packetID=%d|topicName=%s|payload=%s", pParams->id, topicName,
                  STRING_PTR_PRINT_SANITY_CHECK((char *)pParams->payload));
        } else {
            ATSVRLOG("publish packetID=%d|topicName=%s", pParams->id, topicName);
        }*/
         ATSVRLOG("publish packetID=%d|topicName=%s|payload=%s", pParams->id, topicName,
                  STRING_PTR_PRINT_SANITY_CHECK((char *)pParams->payload));
    }
    ...
    if (pParams->qos > QOS0) {
        ...
        if (ATSVR_RET_SUCCESS != rc) {
            ATSVRLOG("push publish into to pubInfolist failed!");
            HAL_MutexUnlock(pClient->lock_write_buf);
            AT_FUNC_EXIT_RC(rc);
        }
    }
