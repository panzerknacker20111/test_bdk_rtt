...
int qcloud_iot_mqtt_subscribe(Qcloud_IoT_Client *pClient, char *topicFilter, SubscribeParams *pParams)
{
    ...
    // POINTER_SANITY_CHECK(pParams->on_message_handler, ATSVR_ERR_INVAL);
    STRING_PTR_SANITY_CHECK(topicFilter, ATSVR_ERR_INVAL);
...
// qqiot not support
    ...
    char *topic_filter_stored = at_malloc(topicLen + 1);
...
int qcloud_iot_mqtt_resubscribe(Qcloud_IoT_Client *pClient)
{
    ...
    for (itr = 0; itr < MAX_MESSAGE_HANDLERS; itr++) {
        ...
        if (rc < 0) {
            ATSVRLOG("resubscribe failed %d, topic: %s", rc, topic);
            AT_FUNC_EXIT_RC(rc);
        }
    }
